...
# --- Configuration ---
model_id = "Qwen/Qwen3-8B" # Use base model ID
...
# Load adapter weights from Hugging Face Hub
hf_adapter_id = "intelehealth/Qwen3-8B-GRPO"
print(f"Loading adapter weights from {hf_adapter_id}")
model.load_adapter(hf_adapter_id, adapter_name="grpo_adapter") # Assign a name to the adapter
...
training_args = GRPOConfig(
    output_dir="Qwen3-8B-GRPO-continued-FT-10", # User specified output directory
    ...
    max_steps=10, # User specified max steps
...
validation_dataset_path = "validation_grpo_data.jsonl"
...
# Note: The test dataset split from the original training data is kept but not used in the trainer
# You might want to load a separate test dataset if needed.
